...
%%
w_x_in = [(w_Rad(:,1).*sin( w_theta(:,1))) (w_Rad(:,1).*cos( w_theta(:,1)))];
w_y_in = [(w_Rad(:,2).*sin( w_theta(:,2))) (w_Rad(:,2).*cos( w_theta(:,2)))];
w_z_in = [(w_Rad(:,3).*sin( w_theta(:,3))) (w_Rad(:,3).*cos( w_theta(:,3)))];
w_theta = w_theta + [dt*w];
w_x_Fin = [(w_Rad(:,1).*sin( w_theta(:,1))) (w_Rad(:,1).*cos( w_theta(:,1)))];
w_y_Fin = [(w_Rad(:,2).*sin( w_theta(:,2))) (w_Rad(:,2).*cos( w_theta(:,2)))];
w_z_Fin = [(w_Rad(:,3).*sin( w_theta(:,3))) (w_Rad(:,3).*cos( w_theta(:,3)))];
dwx = w_x_Fin - w_x_in;
dwy = w_y_Fin - w_y_in;
dwz = w_z_Fin - w_z_in;
%%

x = Pv(:,1:m+1) + dt*u.*DCv(:,1) + 0.5*dt*a.*DCa(:,1)+ dwz(:,1)'+ dwy(:,2)';
y = Pv(:,m+2:2*(m+1)) + dt*u.*DCv(:,2)+ 0.5*dt*a.*DCa(:,2)+ dwz(:,2)'+dwx(:,2)';
z = Pv(:,2*(m+1)+1:3*(m+1)) + dt*u.*DCv(:,3)+ 0.5*dt*a.*DCa(:,3)+ dwx(:,1)'+dwy(:,1)';
Pv = [x y z];
